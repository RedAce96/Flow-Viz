...
def _regular_grid_interpolator(x_line, y_line, values_2d, method='linear', fill_value=np.nan):
    try:
        from scipy.interpolate import RegularGridInterpolator
    except ImportError:
        x_coords = np.asarray(x_line, dtype=float)
        y_coords = np.asarray(y_line, dtype=float)
        field_values = np.asarray(values_2d, dtype=float)

        if x_coords.ndim != 1 or y_coords.ndim != 1:
            raise ValueError('Structured grid coordinates must be one-dimensional.')
        if field_values.shape != (x_coords.size, y_coords.size):
            raise ValueError(
                'Field values must match the structured grid shape '
                f'({x_coords.size}, {y_coords.size}); got {field_values.shape}.'
            )
        if x_coords.size < 2 or y_coords.size < 2:
            raise ValueError('At least two points are required in each coordinate direction.')
        if np.any(np.diff(x_coords) <= 0.0) or np.any(np.diff(y_coords) <= 0.0):
            raise ValueError('Structured grid coordinates must be strictly increasing.')
        if method not in {'linear', 'nearest'}:
            raise ValueError("interp_method must be either 'linear' or 'nearest'.")

        def interpolate(sample_points):
            ...
            valid_x = point_x[valid_mask]
            valid_y = point_y[valid_mask]

            if method == 'nearest':
                x_upper = np.searchsorted(x_coords, valid_x, side='left')
                x_upper = np.clip(x_upper, 1, x_coords.size - 1)
                x_lower = x_upper - 1
                choose_upper_x = (valid_x - x_coords[x_lower]) >= (x_coords[x_upper] - valid_x)
                x_indices = np.where(choose_upper_x, x_upper, x_lower)

                y_upper = np.searchsorted(y_coords, valid_y, side='left')
                y_upper = np.clip(y_upper, 1, y_coords.size - 1)
                y_lower = y_upper - 1
                choose_upper_y = (valid_y - y_coords[y_lower]) >= (y_coords[y_upper] - valid_y)
                y_indices = np.where(choose_upper_y, y_upper, y_lower)

                result[valid_mask] = field_values[x_indices, y_indices]
                return result[0] if scalar_input else result

            x_upper = np.searchsorted(x_coords, valid_x, side='right')
            x_upper = np.clip(x_upper, 1, x_coords.size - 1)
            x_lower = x_upper - 1

            y_upper = np.searchsorted(y_coords, valid_y, side='right')
            y_upper = np.clip(y_upper, 1, y_coords.size - 1)
            y_lower = y_upper - 1
            ...
            tx = np.divide(valid_x - x0, x1 - x0, out=np.zeros_like(valid_x), where=(x1 != x0))
            ty = np.divide(valid_y - y0, y1 - y0, out=np.zeros_like(valid_y), where=(y1 != y0))

            f00 = field_values[x_lower, y_lower]
            f10 = field_values[x_upper, y_lower]
            f01 = field_values[x_lower, y_upper]
            f11 = field_values[x_upper, y_upper]
...
def extract_surface_normal_profile(
    ...
    surface_y : float, default=0.0
        Baseline physical surface location.
    surface_offset : float, default=0.0
        Additional shift applied to ``surface_y`` before extracting the profile.
        This is useful when a case uses an immersed boundary located away from
        the domain edge.
    y_max : float, optional
        Optional maximum wall-normal distance to keep.
    ...
    surface_angle_deg : float, optional
        Surface angle in degrees, measured counter-clockwise from ``+x``.
        When omitted, the legacy vertical extraction path is used.
    surface_origin : sequence of 2 floats, optional
        Reference point ``(x0, y0)`` on the surface. Required when
        ``surface_angle_deg`` is provided.
    surface_distance : float, optional
        Distance measured along the surface tangent from ``surface_origin`` to
        the normal-extraction point. If omitted, ``x_location`` is interpreted
        as the x-coordinate of the surface point, matching the legacy API.
    n_samples : int, optional
        Number of interpolated points to sample along an angled surface normal.
        If omitted, a value based on grid spacing is used.
    interp_method : {'linear', 'nearest'}, default='linear'
        Interpolation method used for angled surface-normal extraction.
    ...
    if surface_angle_deg is None:
        ...
        effective_surface_y = float(surface_y) + float(surface_offset)
        x_profile = np.full_like(y_line, sampled_x, dtype=float)

        if side.lower() == 'positive':
            mask = y_line >= effective_surface_y
            if y_max is not None:
                mask &= y_line <= effective_surface_y + y_max
            y_profile = y_line[mask]
            field_profile = values_2d[x_index, mask]
            distance = y_profile - effective_surface_y
        else:
            mask = y_line <= effective_surface_y
            if y_max is not None:
                mask &= y_line >= effective_surface_y - y_max
            ...
            distance = effective_surface_y - y_profile

        x_profile = x_profile[: y_profile.size]
        surface_point = np.array([sampled_x, effective_surface_y], dtype=float)
        normal_direction = np.array([0.0, 1.0 if side.lower() == 'positive' else -1.0], dtype=float)
    else:
        ...
        try:
            surface_origin_x, surface_origin_y = [float(value) for value in surface_origin]
        except (TypeError, ValueError) as exc:
            raise ValueError("surface_origin must contain exactly two numeric values: (x0, y0).") from exc

        theta = np.deg2rad(float(surface_angle_deg))
        tangent = np.array([np.cos(theta), np.sin(theta)], dtype=float)
        normal_direction = np.array([-np.sin(theta), np.cos(theta)], dtype=float)
        if side.lower() == 'negative':
            normal_direction = -normal_direction

        if surface_distance is None:
            sampled_x = float(x_location)
            effective_surface_y = surface_origin_y + np.tan(theta) * (sampled_x - surface_origin_x)
            surface_point = np.array([sampled_x, effective_surface_y], dtype=float)
        else:
            surface_point = np.array([surface_origin_x, surface_origin_y], dtype=float) + float(surface_distance) * tangent
            sampled_x = float(surface_point[0])
            effective_surface_y = float(surface_point[1])

        surface_point = surface_point + float(surface_offset) * normal_direction
        sampled_x = float(surface_point[0])
        effective_surface_y = float(surface_point[1])

        x_min = float(np.min(x_line))
        x_max = float(np.max(x_line))
        y_min = float(np.min(y_line))
        y_max_domain = float(np.max(y_line))

        if (
            surface_point[0] < x_min
            or surface_point[0] > x_max
            or surface_point[1] < y_min
            or surface_point[1] > y_max_domain
        ):
            raise ValueError(
                'The requested surface-normal origin is outside the available dataset bounds: '
                f'surface_point=({surface_point[0]:.6e}, {surface_point[1]:.6e}), '
                f'x_bounds=({x_min:.6e}, {x_max:.6e}), '
                f'y_bounds=({y_min:.6e}, {y_max_domain:.6e}).'
            )

        exit_distances = []
        tol = 1.0e-14
        if normal_direction[0] > tol:
            exit_distances.append((x_max - surface_point[0]) / normal_direction[0])
        elif normal_direction[0] < -tol:
            exit_distances.append((x_min - surface_point[0]) / normal_direction[0])

        if normal_direction[1] > tol:
            exit_distances.append((y_max_domain - surface_point[1]) / normal_direction[1])
        elif normal_direction[1] < -tol:
            exit_distances.append((y_min - surface_point[1]) / normal_direction[1])

        exit_distances = [float(distance_value) for distance_value in exit_distances if distance_value >= 0.0]
        if len(exit_distances) == 0:
            raise ValueError(
                'Could not determine a valid in-domain surface-normal segment. '
                'Check the supplied surface_origin, surface_angle_deg, and side.'
            )

        max_distance = min(exit_distances)
        if y_max is not None:
            max_distance = min(max_distance, float(y_max))

        if max_distance <= 0.0:
            raise ValueError(
                'The requested wall-normal interval is empty after applying the '
                'surface geometry and domain bounds.'
            )

        x_spacing = np.diff(np.asarray(x_line, dtype=float))
        y_spacing = np.diff(np.asarray(y_line, dtype=float))
        positive_spacings = np.concatenate([
            x_spacing[x_spacing > 0.0],
            y_spacing[y_spacing > 0.0],
        ])
        if positive_spacings.size > 0:
            min_spacing = float(np.min(positive_spacings))
        else:
            min_spacing = max_distance / 200.0

        if n_samples is None:
            n_samples = max(2, int(np.ceil(max_distance / min_spacing)) + 1)
        else:
            n_samples = max(2, int(n_samples))

        distance = np.linspace(0.0, max_distance, n_samples, dtype=float)
        sample_points = surface_point[None, :] + distance[:, None] * normal_direction[None, :]

        interpolator = _regular_grid_interpolator(
            x_line,
            y_line,
            values_2d,
        ...
        field_profile = interpolator(sample_points)

        finite_mask = np.isfinite(field_profile)
        if not np.any(finite_mask):
            raise ValueError(
                'Interpolation along the requested surface normal returned no valid points. '
                'Check the requested offset and surface geometry.'
            )
        ...
        first_invalid = np.flatnonzero(~finite_mask)
        if first_invalid.size > 0:
            end_index = int(first_invalid[0])
            distance = distance[:end_index]
            sample_points = sample_points[:end_index]
            field_profile = field_profile[:end_index]

        x_profile = sample_points[:, 0]
        y_profile = sample_points[:, 1]

    if y_profile.size == 0:
        raise ValueError(
            'No points were selected for the requested wall-normal interval. '
            'Check x_location, surface_y, surface_offset, surface geometry, and y_max.'
        )
    ...
    return {
        ...
        'surface_y': effective_surface_y,
        'surface_offset': float(surface_offset),
        'surface_angle_deg': None if surface_angle_deg is None else float(surface_angle_deg),
        'surface_origin': None if surface_origin is None else (
            float(surface_origin[0]),
            float(surface_origin[1]),
        ),
        'surface_distance': None if surface_distance is None else float(surface_distance),
...
def plot_surface_normal_profiles(
    ...
    field_label : str, optional
        Horizontal-axis label. Defaults to the field key of the first profile.
    title : str, optional
        Figure title.
    output_path : str, optional
        If provided, save the figure to this path.
    x_key : str, default='distance_from_surface'
        Vertical-axis key to plot from each profile.
    ...
    ax.set_xlabel(field_label, fontsize=14)
